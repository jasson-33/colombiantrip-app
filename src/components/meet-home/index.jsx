  ...
  return (
    <section className={`siteSection ${styles.meetSection}`}>
      <div className={`container ${styles.containerMeetSction}`}>
        <h2 className={`${styles.meetHomeTitle} ${Bigola.className}`} dangerouslySetInnerHTML={{__html:title}} />
        <h3 className={`${styles.subtitleMeetHome}`} dangerouslySetInnerHTML={{__html:subtitle}} />
        ...
        <div className={styles.blogsContainer}>
          {blogs.map((blog, i) => (
            <div key={typeof window !== 'undefined' ? window.crypto.randomUUID() : i} className={styles.cardBlog}>
              ...
              <Link href={`/meet-colombia/${blog.url}`} className={styles.readMoreAnchor}><span className={`bg-ct ${styles.arrowRight}`}></span> Read More</Link>
            </div>
          ))}
        </div>
        <Link href='/meet-colombia' className={`${styles.allArticles} ${styles.readMoreAnchor}`}><span className={`bg-ct ${styles.arrowRight}`}></span>VIEW ALL ARTICLES</Link>
      </div>
    </section>
  );
};
